...
export function ProjectModal({ children }: { children: React.ReactNode }) {
  ...
  return (
    ...
    <div
      className="modal-backdrop fixed inset-0 z-[60] overflow-y-auto bg-ink/80 backdrop-blur-sm"
      onClick={(event) => {
    ...
    >
      <div className="flex min-h-full items-start justify-center p-4 py-10 md:py-16">
        <div
          ...
          aria-modal="true"
          tabIndex={-1}
          className="modal-panel relative w-full max-w-3xl rounded-xl border border-line bg-surface p-6 shadow-2xl shadow-black/60 outline-none md:p-10"
        >
          <button
            type="button"
            onClick={() => router.back()}
            aria-label="Close project"
            className="absolute top-4 right-4 flex size-9 items-center justify-center rounded-full border border-line font-mono text-sm text-muted transition-colors hover:border-amber hover:text-amber"
          >
            ✕
          </button>
          {children}
